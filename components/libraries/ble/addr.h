...
typedef struct bt_addr_le
{
    uint8_t type;
    bt_addr_t a;
} bt_addr_le_t;

/* Global Bluetooth address constants defined in bluetooth/bt_common/addr.c */
...
{
    return snprintf(str, len, "%02X:%02X:%02X:%02X:%02X:%02X", addr->val[5], addr->val[4],
...
static inline int bt_addr_le_to_str(const bt_addr_le_t *addr, char *str, size_t len)
{
    ...
    return snprintf(str, len, "%02X:%02X:%02X:%02X:%02X:%02X (%s)", addr->a.val[5], addr->a.val[4],
                    addr->a.val[3], addr->a.val[2], addr->a.val[1], addr->a.val[0], type);
}
